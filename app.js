/* Mobile Sangam — app.js (Secure Input Validation vFinal)
   Features:
   - Preload relations & countries
   - Lock selectors after Start, unlock on Restart
   - Name inputs: English A–Z only (auto Uppercase)
   - Mobile inputs: digits only, country-wise length guard
   - Block identical mobile numbers
   - PDF export + Diamond CTA preserved
*/
const MS = (()=>{
  ...
  const bindNameGuards=()=>{
    ['nameA','nameB'].forEach(id=>{
      ...
      el.addEventListener('paste',(e)=>{ e.preventDefault(); const t=(e.clipboardData.getData('text')||'').replace(/[^A-Za-z]/g,'').toUpperCase(); el.value=t; });
    });
  };
  const bindMobileGuards=()=>{
    ['mobileA','mobileB'].forEach(id=>{
      ...
      el.addEventListener('input',()=>{
        ...
        const max=S.country?.mobile?.maxLen || 15;
      ...
      el.addEventListener('paste',(e)=>{
        ...
        const max=S.country?.mobile?.maxLen || 15;
        el.value = t.slice(0,max);
      });
    });
  };
  const syncMobileAttrs=()=>{
    const min=S.country?.mobile?.minLen||10;
    const max=S.country?.mobile?.maxLen||10;
    ['mobileA','mobileB'].forEach(id=>{
      const el=document.getElementById(id); if(!el) return;
      el.setAttribute('maxlength', String(max));
      el.setAttribute('placeholder', `Enter ${min}-${max} digits`);
    });
  };
  ...
  const calcPair=({nameA,nameB,mA,mB})=>{
    ...
    let score=100; score-=Math.abs(A.naamank-B.naamank)*8; score-=Math.abs(A.yogank-B.yogank)*5; score=clamp(Math.round(score),0,100);
  ...
  const exportPDF=async()=>{
    const node=qs('#reportContainer');
    if(!node||!node.innerText.trim()){ alert(LANG()==='en'?'Please generate the report first.':'पहले रिपोर्ट जनरेट करें।'); return; }
  ...
  const start=async()=>{ await loadCountriesIndex(); await loadAll(); setStage(1); lockSelectors(true); };
  const generate=()=>{
    ...
    if(!nA||!nB) return warn(LANG()==='en'?'Enter valid names (A–Z only)':'वैध नाम लिखें (A–Z)');
